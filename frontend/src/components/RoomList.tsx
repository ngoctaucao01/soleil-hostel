...
const RoomCard: React.FC<RoomCardProps> = ({ room }) => (
  <div className="bg-white rounded-xl shadow-lg p-6 flex flex-col justify-between hover:scale-105 transition-transform duration-200">
    <div>
      ...
      {/* Giá phòng */}
      <p className="mb-1 text-gray-700">Price: <span className="font-bold text-green-600">${room.price}</span></p>
      {/* Số khách tối đa */}
      <p className="mb-1 text-gray-700">Max Guests: <span className="font-bold">{room.max_guests}</span></p>
      {/* Trạng thái phòng với màu sắc động */}
      <p className={
        `inline-block px-3 py-1 rounded-full text-sm font-medium mt-2 ` +
        (room.status === 'available' ? 'bg-green-100 text-green-700' : room.status === 'booked' ? 'bg-yellow-100 text-yellow-700' : 'bg-red-100 text-red-700')
      }>
    ...
    return (
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
          {Array.from({ length: 6 }).map((_, idx) => <RoomSkeleton key={idx} />)}
        </div>
      </div>
    );
  }
